 ...
 * @param[in] adc_1         ADC1 value.
 * @param[in] adc_2         ADC2 value.
 * @param[in] adc_3         ADC3 value.
 * @param[in] max_bit       EEPROM reg address max bit, if EEPROM address more then 512Kb, max_bit=ture.
 */
void write_EEPROM_data(uint8_t reg_byte_H, uint8_t reg_byte_L, uint8_t adc_1, uint8_t adc_2, uint8_t adc_3, bool max_bit)
{
	uint8_t EEPROM_addr;
    ret_code_t err_code;
	twi_tx_done=false;
    uint8_t reg[5] = {reg_byte_H, reg_byte_L, adc_1, adc_2, adc_3};
...
void read_EEPROM_data(uint8_t reg_byte_H, uint8_t reg_byte_L, bool max_bit, uint8_t *data)
{
	...
	err_code = nrf_drv_twi_rx(&m_twi, EEPROM_addr, &data[0], 255);
	APP_ERROR_CHECK(err_code);
	nrf_delay_us(500);
	while (twi_rx_done == false);

	/*for(i=0;i<252;i++)//printf page data
	{
		printf(" %d ",data[i]);
		nrf_delay_ms(1);
	}*/
