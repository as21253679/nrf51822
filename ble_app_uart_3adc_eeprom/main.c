...
int main(void)
{
	...
	// eeprom init
	twi_init();//i2c init
	//ERASE_EEPROM_data();//earse eeprom
	...
	while(1)
	{
		if(!nrf_gpio_pin_read(12))
		{
			...
			nrf_delay_ms(5);*/
			//////////////////////////////////////////////////
			ble_nus_string_send(&m_nus, adc_string, strlen(adc_string));//send data through ble uart		
			nrf_delay_ms(500);
		}
		//power_manage();// enter into sleep mode,need use gpio interrupt to wake up device
    }
}
